  ...
  try {
    ...
    const execResult = await execFlyMachineCommand({
      ...
      command: [
        'sh',
        '-lc',
        `if command -v runuser >/dev/null 2>&1; then runuser -u node -- openclaw pairing approve discord ${code}; elif command -v su >/dev/null 2>&1; then su -s /bin/sh node -c 'openclaw pairing approve discord ${code}'; else openclaw pairing approve discord ${code}; fi`,
      ],
      timeout: 30,
    });
    ...
    console.log('approve discord pairing failed:', e);
    return respErr(e.message || 'approve discord pairing failed');
  }
}
