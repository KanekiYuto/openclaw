...
export async function GET() {
  try {
    ...
    const pendingDeploys = await listBotPendingDeploysByUser(user.id, 20);
    ...
    console.log('get dashboard overview failed:', e);
    return respErr(`get dashboard overview failed: ${e.message}`);
  }
}
